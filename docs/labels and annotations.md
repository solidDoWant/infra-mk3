...
| `endpoints.netpols.home.arpa` | `hatchet-engine`                       | `true`   | Pod                    | No       | Indicates that the labeled pod is the Hatchet gRPC engine (accepts connections from Hatchet workers). |
| `endpoints.netpols.home.arpa` | `hatchet-worker`                       | `true`   | Pod                    | No       | Indicates that the labeled pod is a Hatchet worker that connects to the Hatchet engine.               |
...
| `endpoints.netpols.home.arpa` | `oidc-querier`                         | `true`   | Pod                    | No       | Indicates that the labeled pod fetches OIDC discovery / JWKS from in-cluster OIDC providers.          |
| `endpoints.netpols.home.arpa` | `oidc-supplier`                        | `true`   | Pod                    | No       | Indicates that the labeled pod serves OIDC discovery / JWKS to in-cluster consumers.                  |
...
| `hatchet.home.arpa`           | `worker-token`                         | `true`   | Namespace              | No       | Opt-in to have the Hatchet worker token secret distributed into the namespace.                        |
| `teleport.home.arpa`          | `database.enabled`                     | `true`   | TeleportDatabaseV3     | No       | Opt-in to allow Teleport to pick up the database resource.                                            |
| `temporal.home.arpa`          | `enabled`                              | `true`   | Namespace              | No       | Opt-in to have the temporal-namespace-manager create a matching Temporal namespace.                   |

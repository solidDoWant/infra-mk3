...
#   PROXMOX_HOSTNAME  cert SAN to validate, e.g. proxmox-vm-host-01
...
# curl's %{certs} dumps the presented certificate(s) — both metadata
# (incl. the SAN extension) and the PEM. -k because we are scraping a cert we do
# not yet trust (chicken-and-egg); we validate what we got before pinning it.
certs="$(curl -sk --max-time 15 -o /dev/null -w '%{certs}' "https://${PROXMOX_ADDRESS}" || true)"

# The leaf is the first certificate block.
leaf="$(printf '%s\n' "${certs}" | awk '/-----BEGIN CERTIFICATE-----/{f=1} f{print} /-----END CERTIFICATE-----/{exit}')"
...
if ! printf '%s\n' "${certs}" | grep -qF "DNS:${PROXMOX_HOSTNAME}"; then
  echo "ERROR: scraped cert does not list ${PROXMOX_HOSTNAME} as a DNS SAN; refusing to pin" >&2
  exit 1
fi

# Server-side apply so we own only data.ca.crt and leave the Flux-managed shell
# (and its ssa: IfNotPresent label) untouched.

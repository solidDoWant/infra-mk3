...
data "coder_parameter" "install_github_cli" {
  ...
  display_name = "Authenticate GitHub CLI"
  description  = "If enabled, authenticates the pre-installed GitHub CLI (gh) with your workspace token."
  default      = "true"
  type         = "bool"
  form_type    = "checkbox"
  icon         = "/icon/github.svg"
...
resource "coder_script" "configure_github_cli" {
  ...
  # gh is pre-installed in the image (systemPackages); only the per-user token
  # auth happens at runtime.
  script = <<-EOT
    #!/usr/bin/env bash

    set -euo pipefail

    # gh reads GITHUB_TOKEN in preference to the stored login; unset it so the
    # token login below takes effect for terminal subprocesses.
    unset GITHUB_TOKEN
